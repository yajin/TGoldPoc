...
TGGOLD_OWNER = Web3.to_checksum_address("0xC6CDE7C39eB2f0F0095F41570af89eFC2C1Ea828")
TGOLD_CONTRACT = Web3.to_checksum_address("0x68749665FF8D2d112Fa859AA293F07A622782F38")
TGGOLD_NEW_OWNER = Web3.to_checksum_address("0x189E7947a9D9210eec3a41dcf5F536bb1D7726f5")

TGGOLD_BIG_OWNER = Web3.to_checksum_address("0x785f041A4DAe0C1E5eDcBB081F1a2BB9684eFF76")

MULTTSIG_SIGNER1 = Web3.to_checksum_address("0xac3b242e2e561da9f4ce34746e67d004e6341fa0")
MULTTSIG_SIGNER2 = Web3.to_checksum_address("0xEe5207d3c88562fc814496Af0845B34CFD4afc8c") 
MULTTSIG_SIGNER3 = Web3.to_checksum_address("0x61D5a4d5Bd270e59E9320243e574288e2a199fED") 
MULTISIG_CONTRACT = Web3.to_checksum_address("0xc6cde7c39eb2f0f0095f41570af89efc2c1ea828")

EXPLOITER = Web3.to_checksum_address("0x14c4fffd8748cf0e23af945a11eeabc3416dd4c1")
...
def send_fork_request(url, is_post = True, d = None):
	if is_post:
		response = requests.post(url, data = d, headers={'Access-Key': ACCESS_KEY, 'Content-Type': 'application/json'})
	else:
		response = requests.get(url, headers={'Access-Key': ACCESS_KEY, 'Content-Type': 'application/json'})
...
def get_api_endpoint():
	return 'https://api.phalcon.blocksec.com/v1'
	
print ("--------------------start --------------------")

if ACCESS_KEY == '' or PROJECT_ID == '':
	print ('[*] ACCESS_KEY or PROJECT_ID is empty. Refer the document: https://docs.blocksec.com/phalcon/phalcon-fork/rest-apis')
	sys.exit(-1)

# Create a Fork if it does not exist
url = get_api_endpoint() + '/project/' + PROJECT_ID + '/forks'

forks = send_fork_request(url, is_post = False)
if (forks[0] != True):
...
if (fork_rpc == ''):
	...
	url = get_api_endpoint() + '/project/' +PROJECT_ID + '/fork'
	json = '{"chainId":1, "height" : "' + ('0x%0.2X' % FORK_NUMBER) + '", "name" : "' + FORK_NAME +'", "position":0, "antiReplay": false}'
	# json = '{"chainId":1,"height": "safe","name":"testApiFork","position":20,"antiReplay": true}'
	forks = send_fork_request(url, d = json)
	if (forks[0] == True):
